...
libname c 'C:\Users\eleonora.bonafe2\Desktop\sas';
...
data c.wghtclub3; set c.wghtclub2;
if loss>mean_team then result="better than your team!";
if loss=mean_team then result="on average!";
if loss<mean_team then result="keep training!";
run;


data kg_club; set c.wghtclub3;
strtkg=strtwght/2.2;
endkg=endwght/2.2;
losskg=loss/2.2;
run;
...
/**/
 data kg_club1; set c.wghtclub3;
 array b strtwght endwght loss;
 array b1 strtkg  endkg losskg;
 do over b1;
 b1=b/2.2;
 end;
 run;

proc transpose data=c.wghtclub3 out=wghtclubt; 
